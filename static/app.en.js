...
(function () {
  ...
  function scaleOf(hex) {
    ...
    wrap.setAttribute('aria-label', 'SHA-256 fingerprint of the sealed document');
    for (var i = 0; i < 64; i++) {
  ...
  function panel(o) {
    ...
    function fromFile(f) {
      ...
      f.arrayBuffer().then(sha256).then(function (h) {
        ...
        set(null, 'the file could not be read');
      });
    }
    ...
    text.addEventListener('input', function () {
      ...
      var v = text.value;
      if (!v) { set(null, 'no input'); return; }
      sha256(new TextEncoder().encode(v)).then(function (h) {
        set(h, v.length + ' characters');
      });
    });
  ...
  if (!subtle) {
    ...
    document.querySelectorAll('.privacy span:last-child').forEach(function (el) {
      el.textContent = 'This browser cannot compute the fingerprint here: a secure connection (https) is required.';
    });
    return;
  }
  ...
  $('btn-s').addEventListener('click', function () {
    var box = $('res-s');
    S.busy(true);
    box.innerHTML = '<div class="result"><span class="tag sig"><span class="spin"></span>Sending to the calendars</span>'
                  + '<h2>Sealing…</h2></div>';
    ...
    fetch('/api/seal', {
      ...
      .then(function (res) {
        ...
        if (!res.ok) {
          box.innerHTML = '<div class="result bad"><span class="tag sig">Error</span>'
                        + '<h2>Could not seal</h2><p></p></div>';
          box.querySelector('p').textContent = res.b.error || 'Unknown error.';
          return;
        }
        var s = res.b;
        box.innerHTML =
          ...
          + '<h2>Sealed</h2>'
          + '<p>It stays <b>pending</b> until it enters a Bitcoin block, usually a few hours. '
          + 'It is already valid from now: the recorded date is this one.</p>'
          + '<div class="j-scale"></div>'
          + '<div class="scale-legend"><span class="tag">digit 1</span>'
          +   '<span class="tag">this is the fingerprint that has been recorded</span>'
          +   '<span class="tag">64</span></div>'
          + '<dl>'
          +   '<div><dt>Record</dt><dd class="j-id"></dd></div>'
          +   '<div><dt>Sealed</dt><dd class="j-at"></dd></div>'
          +   '<div><dt>Fingerprint</dt><dd class="j-h"></dd></div>'
          + '</dl>'
          + '<div class="actions"><a class="btn btn-2 j-p" href="#" download>Download .ots proof</a></div>'
          + '</div>';
      ...
      .catch(function (e) {
        S.busy(false);
        box.innerHTML = '<div class="result bad"><h2>Could not seal</h2><p></p></div>';
  ...
  $('btn-v').addEventListener('click', function () {
    var box = $('res-v');
    V.busy(true);
    box.innerHTML = '<div class="result"><span class="tag sig"><span class="spin"></span>Querying</span>'
                  + '<h2>Checking…</h2></div>';
    ...
    fetch('/api/verify', {
      ...
      .then(function (res) {
        ...
        if (!res.found) {
          box.innerHTML =
            '<div class="result bad"><span class="tag sig">No match</span>'
            + '<h2>No seal on record</h2>'
            + '<p>This fingerprint is not in the registry. If you expected to find it, the content has '
            + 'changed: one character is enough for the fingerprint to be a different one.</p></div>';
          return;
        }
        var filas = res.stamps.map(function (s) {
          var estado = s.status === 'anchored'
            ? 'anchored in Bitcoin' + (s.bitcoin_heights.length ? ' · block ' + s.bitcoin_heights.join(', ') : '')
            : 'pending anchoring';
          return '<dl>'
            + '<div><dt>Record</dt><dd>' + esc(s.id) + '</dd></div>'
            + '<div><dt>Sealed</dt><dd>' + esc(s.created_at) + '</dd></div>'
            + '<div><dt>Status</dt><dd>' + estado + '</dd></div>'
            + (s.label ? '<div><dt>Label</dt><dd class="j-l"></dd></div>' : '')
            + '</dl>';
        }).join('');
        box.innerHTML =
          '<div class="result ok"><span class="tag ok">Match</span>'
          + '<h2>Seal found</h2>'
          + '<p>This exact content was recorded on the date shown.</p>'
          + filas + '</div>';
      ...
      .catch(function (e) {
        V.busy(false);
        box.innerHTML = '<div class="result bad"><h2>Error while checking</h2><p></p></div>';
        box.querySelector('p').textContent = e.message;
      });
  });
})();
